...
# DÜZELTME: QPointF eklendi
from PyQt6.QtCore import Qt, QTimer, QPropertyAnimation, QEasingCurve, QPoint, QPointF, QRectF
...
class PulsingMicWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.is_listening:
            color = QColor(0, 255, 255, int(max(0, self.ripple_opacity))) # Cyan
        else:
            color = QColor(80, 80, 80, int(max(0, self.ripple_opacity))) # Gri (Bekleme modu)

        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(QBrush(color))
        
        # DÜZELTME BURADA YAPILDI:
        # center (QPoint) -> QPointF(center) olarak çevrildi.
        # Böylece float olan ripple_radius ile uyumlu hale geldi.
...
class ModernAtlasWindow(QMainWindow):
    def __init__(self):
        ...
        # Eğer sistemde Orbitron yoksa Arial kullanır
        ...
        # Test için: Mikrofona tıklayınca durumu değiştir
        self.mic_widget.mousePressEvent = self.toggle_listening
